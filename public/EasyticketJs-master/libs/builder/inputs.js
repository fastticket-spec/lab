...
var Input = {
	...
	renderTemplate: function(name, data) {
		return tmpl("easyticket-input-" + name, data);
	},
...
var CssUnitInput = $.extend({}, Input, {
	...
	setValue: function(value) {
		...
		this.unit = value.replace(this.number, 'px');
...
var ImageInput = $.extend({}, Input, {
	...
	onUpload: function(event, node) {
		...
		function imageIsLoaded(e) {
				...
				$.ajax({
					type: "POST",
					url: document.location.origin+'/event_badge_image_upload',//set your server side upload script url
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
						...
						// alert(data.details);
						event.data.element.trigger('propertyChange', [data.details, this]);

						//update src input
						$('input[type="text"]', event.data.element).val(data.details);
					},
					error: function (data) {
