...
https://github.com/givanz/EasyticketJs
*/
...
if (Easyticket === undefined) var Easyticket = {};

Easyticket.defaultComponent = "_base";
Easyticket.preservePropertySections = true;
Easyticket.dragIcon = 'icon';//icon = use component icon when dragging | html = use component html to create draggable element

Easyticket.baseUrl =  document.currentScript?document.currentScript.src.replace(/[^\/]*?\.js$/,''):'';

Easyticket.ComponentsGroup = {};
Easyticket.BlocksGroup = {};

Easyticket.Components = {
...
*/
	...
	render: function(type) {
		...
		if (!(Easyticket.preservePropertySections && section.length))
		{
			componentsPanelSections[defaultSection].html('').append(tmpl("easyticket-input-sectioninput", {key:"default", header:component.name}));
		...
		if (component.beforeInit) component.beforeInit(Easyticket.Builder.selectedEl.get(0));
		...
		var fn = function(component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
					...
					var element = Easyticket.Builder.selectedEl;
					...
					if (property.htmlAttr)
					{
						...
						{
							element = Easyticket.StyleManager.setStyle(element, property.key, value);
						}
						else if (property.htmlAttr == "innerHTML")
						{
							element = Easyticket.ContentManager.setHtml(element, value);
						}
						else
						{
						...
						Easyticket.Undo.addMutation({type: 'attributes',
					...
					if (!property.child && !property.parent) Easyticket.Builder.selectNode(element);

					return element;
			});
		};

		var nodeElement = Easyticket.Builder.selectedEl;
		...
		for (var i in component.properties)
		{
			...
			} else if (property.htmlAttr)
			{
				if (property.htmlAttr == "style")
				{
					...
					var value = Easyticket.StyleManager.getStyle(element, property.key);//getStyle returns declared style
				} else
				if (property.htmlAttr == "innerHTML")
				{
					var value = Easyticket.ContentManager.getHtml(element);
				} else
				{
			...
			{
				var row = $(tmpl('easyticket-property', property));
		...
		if (component.init) component.init(Easyticket.Builder.selectedEl.get(0));
	}
};
...
Easyticket.WysiwygEditor = {
	...
	destroy: function(element) {
		...
		Easyticket.Undo.addMutation({type:'characterData',
								target: node,
...
/* controls */
	loadControlGroups : function() {
		...
		for (group in Easyticket.ComponentsGroup)
		{
			...
			components = Easyticket.ComponentsGroup[ group ];

			for (i in components)
			{
				componentType = components[i];
				component = Easyticket.Components.get(componentType);
	...
	loadBlockGroups : function() {
		...
		for (group in Easyticket.BlocksGroup)
		{
			...
			blocks = Easyticket.BlocksGroup[ group ];

			for (i in blocks)
			{
				blockType = blocks[i];
				block = Easyticket.Blocks.get(blockType);
	...
	loadUrl : function(url, callback) {
		...
		self.initCallback = callback;
		if (Easyticket.Builder.iframe.src != url) Easyticket.Builder.iframe.src = url;
	},
...
/* iframe */
	_loadIframe : function(url) {
	    ...
	    return this.documentFrame.on("load", function()
        {
				...
				$(window.FrameWindow).on( "beforeunload", function(event) {
					if (Easyticket.Undo.undoIndex <= 0)
					{
				...
				Easyticket.WysiwygEditor.init(window.FrameDocument);
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append('<link data-easyticket-helpers href="' + Easyticket.baseUrl + '../../css/easyticketjs-editor-helpers.css" rel="stylesheet">');

		self._initHighlight();

		$(window).triggerHandler("easyticket.iframe.loaded", self.frameDoc);
    },
	...
	loadNodeComponent:  function(node) {
		data = Easyticket.Components.matchNode(node);
		var component;

		if (data)
			component = data.type;
		else
			component = Easyticket.defaultComponent;

		Easyticket.Components.render(component);
	...
	selectNode:  function(node) {
		...
		{
			Easyticket.WysiwygEditor.destroy(self.texteditEl);
...
/* iframe highlight */
    _initHighlight: function() {
		...
		self.frameHtml.on("mouseup touchend", function(event) {
			if (self.isDragging)
			{
				...
				if (self.dragMoveMutation === false)
				{
					Easyticket.Undo.addMutation({type: 'childList',
				...
				} else
				{
					...
					Easyticket.Undo.addMutation(self.dragMoveMutation);
		...
		self.frameHtml.on("dblclick", function(event) {
			...
			if (Easyticket.Builder.isPreview == false)
			{
				self.texteditEl = target = jQuery(event.target);

				Easyticket.WysiwygEditor.edit(self.texteditEl);
	...
	_initBox: function() {
		...
		$("#down-btn").on("click", function(event) {
			...
			Easyticket.Undo.addMutation({type: 'move',
									target: node,
									oldParent: oldParent,
									newParent: newParent,
		...
		$(".components-list li ol li", addSectionBox).on("click", function(event) {
			var html = Easyticket.Components.get(this.dataset.type).html;
		...
		$(".blocks-list li ol li", addSectionBox).on("click", function(event) {
			var html = Easyticket.Blocks.get(this.dataset.type).html;
...
/* drag and drop */
	_initDragdrop : function() {
		...
		$('.drag-elements-sidepane ul > li > ol > li').on("mousedown touchstart", function(event) {
			...
			if ($this.data("drag-type") == "component")
				self.component = Easyticket.Components.get($this.data("type"));
			else
				self.component = Easyticket.Blocks.get($this.data("type"));
	...
	removeHelpers: function (html, keepHelperAttributes = false)
	{
		...
		html = html.replace(/<.*?data-easyticket-helpers.*?>/gi, "");
		//attributes
		if (!keepHelperAttributes)
		{
			html = html.replace(/\s*data-easyticket-\w+(=["'].*?["'])?\s*/gi, "");
		}
	...
	getHtml: function(keepHelperAttributes = true)
	{
		...
		$(window).triggerHandler("easyticket.getHtml.before", doc);
		...
		"<!DOCTYPE "
         ...
         var filter = $(window).triggerHandler("easyticket.getHtml.after", html);
         if (filter) return filter;

         return html;
    },

    getHtmlForPdf: function(keepHelperAttributes = true)
	{
		var doc = window.FrameDocument;
		var hasDoctpe = (doc.doctype !== null);
		var html = "";
        ...
        //  doc.document
         ...
         return doc.getElementById('badgeContainer');
    },

    getHtmlForPdfWidth: function(keepHelperAttributes = true)
	{
		var doc = window.FrameDocument;
		var hasDoctpe = (doc.doctype !== null);
		var html = "";
        ...
        //  doc.document
         ...
         return doc.getElementById('hold').style.width.replace('px', '');
    },

    getHtmlForPdfHeight: function(keepHelperAttributes = true)
	{
		var doc = window.FrameDocument;
		var hasDoctpe = (doc.doctype !== null);
		var html = "";
        ...
        //  doc.document
         ...
         return doc.getElementById('hold').style.height.replace('px', '');
	},
	...
	saveAjax: function(fileName, startTemplateUrl, badge_id, event_id, store_url, callback)
	{
		var data = {};
		var tokens=$('meta[name="csrf-token"]').attr('content');
		// alert(tokens);
		data["fileName"] = (fileName && fileName != "") ? fileName : Easyticket.FileManager.getCurrentUrl();
		...
		$.ajax({
			type: "POST",
			url: store_url,//set your server side save script url
			data: data,
			cache: false,
			success: function (data) {
				if (callback) callback(data.details);
...
Easyticket.CodeEditor = {
	...
	init: function(doc) {
		$("#easyticket-code-editor textarea").val(Easyticket.Builder.getHtml());

		$("#easyticket-code-editor textarea").keyup(function ()
		{
			delay(Easyticket.Builder.setHtml(this.value), 1000);
		});

		//load code on document changes
		Easyticket.Builder.frameBody.on("easyticket.undo.add easyticket.undo.restore", function (e) { Easyticket.CodeEditor.setValue();});
...
Easyticket.Gui = {
	...
	init: function() {
		$("[data-easyticket-action]").each(function () {
			on = "click";
			if (this.dataset.easyticketOn) on = this.dataset.easyticketOn;

			$(this).on(on, Easyticket.Gui[this.dataset.easyticketAction]);
			if (this.dataset.easyticketShortcut)
			{
				$(document).bind('keydown', this.dataset.easyticketShortcut, Easyticket.Gui[this.dataset.easyticketAction]);
				$(window.FrameDocument, window.FrameWindow).bind('keydown', this.dataset.easyticketShortcut, Easyticket.Gui[this.dataset.easyticketAction]);
			}
		});
	},

	undo : function () {
		if (Easyticket.WysiwygEditor.isActive)
		{
			Easyticket.WysiwygEditor.undo();
		} else
		{
			Easyticket.Undo.undo();
		}
		Easyticket.Builder.selectNode();
	},

	redo : function () {
		if (Easyticket.WysiwygEditor.isActive)
		{
			Easyticket.WysiwygEditor.redo();
		} else
		{
			Easyticket.Undo.redo();
		}
		Easyticket.Builder.selectNode();
	},
	...
	save : function () {
		$('#textarea-modal textarea').val(Easyticket.Builder.getHtml());
		$('#textarea-modal').modal();
	},

	//post html content through ajax to save to filesystem/db
	saveAjax : function () {
		var url = Easyticket.FileManager.getCurrentUrl();
		var badge_id = $('#badge_id').val();
		var event_id = $('#event_id').val();
		var store_url = $('#store_url').val();
		return Easyticket.Builder.saveAjax(url, null,badge_id, event_id,store_url, function (data) {
			$('#message-modal').modal().find(".modal-body").html(data);
		});
	},

	download : function () {
		filename = /[^\/]+$/.exec(Easyticket.Builder.iframe.src)[0];
        uriContent = "data:application/octet-stream,"  + encodeURIComponent(Easyticket.Builder.getHtml());
        ...
        var source = Easyticket.Builder.getHtmlForPdf()
        html2canvas(source, {
            useCORS: true,
            allowTaint: true,
            onrendered: function (canvas) {
            // Few necessary setting options
            var imgWidth = Easyticket.Builder.getHtmlForPdfWidth() * 28;
            var pageHeight = 701;
            var imgHeight =  Easyticket.Builder.getHtmlForPdfHeight() * 28;
        ...
        // doc.fromHTML( source,0, 0, { 'width': 118,'elementHandlers': elementHandler },
	...
	toggleEditor : function () {
		$("#easyticket-builder").toggleClass("bottom-panel-expand");
		$("#toggleEditorJsExecute").toggle();
		Easyticket.CodeEditor.toggle();
	},

	toggleEditorJsExecute : function () {
		Easyticket.Builder.runJsOnSetHtml = this.checked;
	},

	preview : function () {
		(Easyticket.Builder.isPreview == true)?Easyticket.Builder.isPreview = false:Easyticket.Builder.isPreview = true;
		$("#iframe-layer").toggle();
		$("#easyticket-builder").toggleClass("preview");
	},
...
//Pages, file/components tree
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( event ) {
			...
			Easyticket.FileManager.addPage(name, title, startTemplateUrl);
			Easyticket.FileManager.loadPage(name);
			Easyticket.FileManager.scrollBottom();
	...
	setDesignerMode : function () {
		...
		Easyticket.Builder.setDesignerMode(designerMode);
	},
	...
	toggleFileManager: function () {
		Easyticket.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
	},

	toggleLeftColumn: function () {
		Easyticket.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
	},

	toggleRightColumn: function () {
		Easyticket.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
		var rightColumnEnabled = this.attributes["aria-pressed"].value == "true";

		$("#easyticket-builder").toggleClass("no-right-panel");
		$(".component-properties-tab").toggle();

		Easyticket.Components.componentPropertiesElement = (rightColumnEnabled ? "#right-panel" :"#left-panel") +" .component-properties";
		if ($("#properties").is(":visible")) $('.component-tab a').show().tab('show');

	},
}

Easyticket.StyleManager = {
...
Easyticket.FileManager = {
	...
	init: function() {
		...
		// 	if (page) Easyticket.FileManager.loadPage(page);
		// 	return false;
		// })
		...
		$(this.tree).on("click", "li[data-component] label ", function (e) {
			...
			Easyticket.Builder.frameHtml.animate({
				scrollTop: $(node).offset().top
			}, 1000);

			Easyticket.Builder.selectNode(node);
			Easyticket.Builder.loadNodeComponent(node);
	...
	addPage: function(name, data) {
		...
		if (data.folder)
		{
			...
			if (!(folder = this.tree.find('li[data-folder="' + data.folder + '"]')).length)
			{
				...
				folder = $(tmpl("easyticket-filemanager-folder", data));
		...
		folder.append(
			tmpl("easyticket-filemanager-page", data));
	},
	...
	addComponent: function(name, url, title, page) {
		$("[data-page='" + page + "'] > ol", this.tree).append(
			tmpl("easyticket-filemanager-component", {name:name, url:url, title:title}));
	},
				...
				if (node.hasChildNodes()) {
					for (var j = 0; j < node.childNodes.length; j++) {
						...
						if (child && child["attributes"] != undefined &&
							(matchChild = Easyticket.Components.matchNode(child)))
						{
	...
	loadPage: function(name, allowedComponents = false, disableCache = true) {
		...
		Easyticket.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1?'&':'?') + Math.random():''),
			function () {
				Easyticket.FileManager.loadComponents(allowedComponents);
			});
	},
